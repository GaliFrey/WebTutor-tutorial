...
* [Метод query](#Метод-query)
  ...
  * [Получаем корневой элемент со всем его содержимым](#Получаем-корневой-элемент-со-всем-его-содержимым)
...
* [Сборник задач](#Сборник-задач)
  ...
  * [Получаем полный путь до подразделения рекурсивно](#Получаем-полный-путь-до-подразделения-рекурсивно)
...
Примеры XML документов в таблице `collaborator` в поле `data`.
...
Метод `query` используется для извлечения элементов. Извлекается искомый элемент и всё его содержимое, включая другие элементы.
...
В результате получим первый элемент `lastname` со всем его содержимым. Номер елемента для получения задается `[1]`.

![task10_result](./img/task10_result.jpg)

### Получаем все XML элементы по имени

Найдем все элементы с именем `name`. В нашем документа всего три тега с таким именем и все они вложены в разные элементы `custom_elem`.
...
В результате получим все найденные элементы по указанному пути, так как мы не указали номер елемента, который нам нужен.
...
Найдем элемент `name` из второго елемента `custom_elem`. Для этого используем явное указание номера элемента.
...
В результате получим элемент `name` из второго по счету елемента `custom_elem`:
...
Конструкция `...custom_elem[name="field3"]...` ищет элемент `custom_elem` в котором находится элемент `name` со значением `field3` и возвращает найденный элемент `custom_elem`:
...
Найдем значения элементов с основными данными сотруднике (ИД, фамилию, дату рождения и флаг уволен сотрудник или нет).
...
Найдем значения кастомных полей с именами `message_sent`, `date_sending_message` и `message_subject`, используя значения их соседних елементов `name`.
...
Найдем данные о последнем совершенном действии `workflow_log_entry` из истории вызова действий документоборота в карточке заявки. Для этой задачи мы используем документ XML заявки, где содержится история переходов с одного этапа на другой.
...
Для получения предпоследнего елемента можно использовать запись вида `../workflow_log_entry[last() - 1/..]`
...
Использование метода `nodes('value') AS Table1(field)` приведет к созданию псевдотаблицы c именем `Table1`, названием поля будет `field`, где каждая строка - это найденный элемент по выражению `Xpath`.
...
Получим три строки с каждым найденным `custom_elem`, оператор `.` в методе `query` выведет все содержимое найденного элемента.
...
В методе `value` используется запрос `XQuery` и побитовая оперция `<<`. Говоря простым языком, конструкция `count( for $s in (/collaborator/custom_elems/custom_elem) where $s << . return(.) )` возвращает количество всех предшествующих элементов относительно текущего, возвращенного методом `nodes`.

То есть, для первого `custom_elem` нет предшествующих элементов, поэтому количество будет равно `0`. Следующим метод `nodes` возвращает второй `custom_elem`, количетсво предшествующих елементов становится `1` и так далее. Обратите внимание, путь до элемента в методе `nodes` полностью соответствует пути в запросе `XQuery`. Так как конструкция считает количество предшествующих элементов, то отсчет начинается с `0`. Так как функция `count` возвращает число, мы можем производить над возвращенным результатом математические операции.

Существуте более простой способ получить порядковые номера элементов, можно использовать оконную функцию `ROW_NUMBER`, которая легче в понимании и даёт аналогичный результат, но работает гораздо медленнее.
...
Метод `modify` измененяет XML документ. Используется преимущесвтенно в выражении `UPDATE` и может за один раз изменить только одно значение.

Для метода существует 3 ключевых оператора:

* **[insert](#insert)**
* **[delete](#Оператор-delete)**
* **[replace value of](#Оператор-replace-value-of)**
...
Оператор `into` добавляет элементы последним потомком. Для точного указания места вставки используются ключевые слова `as first` и `as last`.
...
Для поиска мы использовали функцию `last()` - последний элемент. Функция `text()` указывает на изменение знаечния.
